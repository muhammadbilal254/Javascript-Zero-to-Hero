...
(async function () {
  ...
  function displaySearchResult(result) {
    ...
    result.forEach(function (recipe) {
      ...
      const ListItem = `
            ...
            <div className="description"><b>Languages: </b>${
              recipe.original_language
            }</div>
            <div className="description"><b>Cast: </b>${recipe.cast
              .map((actor) => actor.name)
              .join(", ")}</div>
                <div className="description"><b>Release Date: </b>${
                  recipe.release_date
                }</div>
            `;
  ...
  function loadRecipeDetails(recipe) {
    containerEl.innerHTML = `
            ...
            <h3>Directors:</h3>
            <div className="description">${recipe.directors
              .map((directors) => directors.name)
              .join(", ")}</div>
            <h3>Writers:</h3>
            <div className="description">${recipe.writers
              .map((directors) => directors.name)
              .join(", ")}</div>
        `;
  }
  ...
  for (let i = 0; i < mostWatchedEl.length; i++) {
    mostWatchedEl[i].addEventListener("click", function () {
      // Add code to handle most recent button click
      const WatchedMovies = movies.sort(
